...
const RegistrationPage = () => {
    ...
    const [userCredentials, setUserCredentials] = useState({
        // ... ваши поля формы регистрации
    });

    const handleRegistration = async () => {
        try {
            // Выполните запрос регистрации, получите токен
            const response = await axios.post('http://your-api-endpoint.com/register', userCredentials);
            ...
            <div className="max-w-lg p-8 sm:pb-4 bg-white rounded shadow-lg text-center sm:text-left">
                ...
                <h3 className="text-xl sm:text-2xl font-semibold mb-6 flex flex-col sm:flex-row items-center">
                    <div className="bg-green-200 p-2 rounded-full flex items-center mb-4 sm:mb-0 sm:mr-2">
                        <svg className="text-green-800 inline-block w-5 h-5" fill="none" stroke-linecap="round"
                ...
                <p>I recently released Treact, a <span className="font-bold">free</span> TailwindCSS Component Kit built
                    with React.</p>
                <p className="mt-2">It has 52 different UI components, 7 landing pages, and 8 inner pages prebuilt. And
                    they are customizable!</p>
                <div
                    className="mt-8 pt-8 sm:pt-4 border-t -mx-8 px-8 flex flex-col sm:flex-row justify-end leading-relaxed">
                    <button
                        className="close-treact-popup px-8 py-3 sm:py-2 rounded border border-gray-400 hover:bg-gray-200 transition duration-300">Close
                    </button>
